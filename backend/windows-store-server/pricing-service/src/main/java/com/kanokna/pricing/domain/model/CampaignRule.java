...
public class CampaignRule {
    ...
    public static CampaignRule percentage(BigDecimal percentage, Money maxDiscount) {
        ...
        return new CampaignRule(DiscountType.CAMPAIGN, percentage, maxDiscount);
    }

    public static CampaignRule fixed(Money amount) {
        return new CampaignRule(DiscountType.CAMPAIGN,
            amount.getAmount(), null);
    }

    public Money calculateDiscount(Money subtotal) {
        Money discount = subtotal.multiply(discountValue.divide(new BigDecimal("100")));
