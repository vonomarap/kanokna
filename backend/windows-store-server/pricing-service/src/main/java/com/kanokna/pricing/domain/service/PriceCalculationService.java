...
public class PriceCalculationService {
    ...
    public Quote calculateQuote(PriceBook priceBook, List<String> selectedOptionIds,
        ...
        BigDecimal areaM2 = widthCm.multiply(heightCm).divide(new BigDecimal("10000"));
        Money basePrice = priceBook.getBasePriceEntry().calculateBasePrice(areaM2, currency);
        decisionTrace.add(PricingDecision.of(
            "BA-PRC-CALC-02",
            "BASE_PRICE_FORMULA",
            String.format("area_m2=%.4f * price_per_m2=%.2f = %.2f %s",
                areaM2, priceBook.getBasePriceEntry().getPricePerM2(),
                basePrice.getAmount(), currency)
        ));
        ...
        for (String optionId : selectedOptionIds) {
            priceBook.findPremiumForOption(optionId).ifPresent(premium -> {
                Money premiumAmount = premium.calculatePremium(basePrice);
                premiumLines.add(PremiumLine.of(optionId, premium.getOptionName(), premiumAmount));
                totalPremiums.add(premiumAmount);
            });
        }
        ...
        decisionTrace.add(PricingDecision.of(
            "BA-PRC-CALC-03",
            "OPTION_PREMIUMS",
            String.format("premium_count=%d, total_premium=%.2f %s",
                premiumLines.size(), totalPremiums.getAmount(), currency)
        ));
        ...
        // BA-PRC-CALC-99: Build final quote
        Quote quote = Quote.builder()
            ...
            .tax(tax)
            .total(total)
            .validUntil(Instant.now().plus(Duration.ofMinutes(quoteTtlMinutes)))
        ...
        decisionTrace.add(PricingDecision.of(
            "BA-PRC-CALC-99",
            "QUOTE_CALCULATED",
            String.format("quoteId=%s, total=%.2f %s",
                quote.getQuoteId(), quote.getTotal().getAmount(), currency)
        ));
