...
import com.kanokna.gateway.config.GatewayPaths;
...
public class AuthenticationLoggingFilter implements WebFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        return chain.filter(exchange)
                ...
                .switchIfEmpty(Mono.fromRunnable(() -> logAuth(exchange, null)))
                .then()));
    }
    ...
    private void logAuth(ServerWebExchange exchange, Authentication auth) {
        ...
        boolean adminPath = GatewayPaths.ADMIN_PATH_LIST.stream()
